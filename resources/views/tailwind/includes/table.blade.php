    ...
    <x-slot name="body">
        ...
        @forelse ($rows as $index => $row)
            <x-livewire-tables::table.row
                ...
                wire:key="table-row-{{ \Illuminate\Support\Str::random() }}"
                :url="method_exists($this, 'setTableRowUrl') ? $this->setTableRowUrl($row) : ''"
                :class="
                    ($index % 2 === 0 ?
                    'bg-white dark:bg-gray-700 dark:text-white' . (method_exists($this, 'getTableRowUrl') ? ' hover:bg-gray-100' : '') :
                    'bg-gray-50 dark:bg-gray-800 dark:text-white') .
                    (method_exists($this, 'setTableRowUrl') ? ' hover:bg-gray-100 dark:hover:bg-gray-900 transition' : '') .
