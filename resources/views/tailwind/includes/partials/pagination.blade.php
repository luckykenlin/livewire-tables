                ...
                <span>
                    @if ($paginator->onFirstPage())
                        <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default leading-5 rounded-md dark:bg-gray-700 dark:text-white dark:border-gray-600">
                               @lang('livewire-tables::pagination.previous')
                        </span>
                    @else
                ...
                <span>
                    @if ($paginator->hasMorePages())
                        <button wire:click="nextPage('{{ $paginator->getPageName() }}')" wire:loading.attr="disabled" class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150 dark:bg-gray-700 dark:text-white dark:ring-gray-600 dark:border-gray-600 dark:hover:bg-gray-600">
                             @lang('livewire-tables::pagination.next')
                        </button>
                    @else
            ...
            <div class="hidden md:flex-1 md:flex md:items-center md:justify-between">
                <div>
                    <p class="text-sm text-gray-700 leading-5 dark:text-white">
                        <span class="text-capitalize">@lang('livewire-tables::pagination.showing')</span>
                        <span class="font-medium">{{ $paginator->firstItem() }}</span>
                        <span>@lang('livewire-tables::pagination.to')</span>
                        <span class="font-medium">{{ $paginator->lastItem() }}</span>
                        <span>@lang('livewire-tables::pagination.of')</span>
                        <span class="font-medium">{{ $paginator->total() }}</span>
                        <span>@lang('livewire-tables::pagination.results')</span>
                    </p>
                </div>
                ...
                <div>
                    <span class="relative z-0 inline-flex rounded-md shadow-sm">
                        <span>
                            {{-- Previous Page Link --}}
                            @if ($paginator->onFirstPage())
                                <span aria-disabled="true" aria-label="@lang('livewire-tables::pagination.previous')">
                                    ...
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-{{ $this->numberOfPaginatorsRendered[$paginator->getPageName()] }}-page{{ $page }}">
                                        @if ($page == $paginator->currentPage())
                                            <span aria-current="page">
                                                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500 bg-gray-200 border border-gray-300 cursor-default leading-5 dark:bg-gray-500 dark:text-white dark:border-gray-500">{{ $page }}</span>
                                            </span>
                                        @else
                        ...
                        <span>
                            ...
                            @else
                                <span aria-disabled="true" aria-label="@lang('livewire-tables::pagination.next')">
